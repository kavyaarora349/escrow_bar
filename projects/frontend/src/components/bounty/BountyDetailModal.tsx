...
import { X, Clock, MessageSquare, Trophy, Users, MapPin, Bookmark, ChevronRight, CheckCircle2, Send, User } from 'lucide-react'
    ...
    <>
      {paragraphs.map((p, i) => {
        ...
        if (p.includes('\n- ') || p.startsWith('- ')) {
          ...
          return (
            <div key={i}>
              {heading && <h3 className="text-lg font-bold text-foreground mt-6 mb-2">{heading.replace(/\*\*/g, '')}</h3>}
...
const BountyDetailModal = ({ bounty, onClose }: BountyDetailModalProps) => {
  ...
  const isOwner = !!activeAddress && !!bounty.creatorAddress && activeAddress.toLowerCase() === bounty.creatorAddress.toLowerCase()
  ...
  const fetchOnChainState = async () => {
    ...
    try {
      ...
      if (activeAddress && info.worker.toLowerCase() === activeAddress.toLowerCase() && info.status >= 1) {
      ...
      if (raw.includes('assert failed') || raw.includes('logic eval error')) {
        ...
        if (onChainStatus === 1 || onChainStatus === 2 || onChainStatus === 3) {
          errorMsg = `This bounty has already been claimed${
            onChainStatus === 2 ? ' and work was submitted.' : onChainStatus === 3 ? ' and has been approved.' : ' by another worker.'
          }`
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="font-display text-xl font-bold text-white leading-tight mb-1.5">{bounty.title}</h2>
                  <div className="flex flex-wrap items-center gap-2 text-sm text-slate-400">
                    <span className="font-medium text-slate-200">{bounty.sponsor || bounty.creator}</span>
                    <span className="text-slate-700">|</span>
                    <span className="flex items-center gap-1">
                      <Trophy className="h-3.5 w-3.5 text-blue-500" /> Bounty
                    </span>
                    <span className="text-slate-700">|</span>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button className="flex items-center gap-1.5 rounded-lg border border-[#27272a] px-3 py-1.5 text-sm text-slate-400 hover:text-white hover:border-slate-600 transition-colors bg-[#18181b]">
                  <Bookmark className="h-4 w-4" /> Bookmark
                </button>
                <button
                  onClick={onClose}
                  className="h-8 w-8 rounded-full bg-[#27272a] flex items-center justify-center text-slate-400 hover:text-white transition-colors"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-72 shrink-0 border-b md:border-b-0 md:border-r border-[#27272a] p-6 md:p-8 space-y-6 bg-[#18181b]/30">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <span className="font-display text-xl font-bold text-white">{bounty.totalPrize || bounty.reward}</span>
                    <span className="block text-xs text-slate-500">Total Prizes</span>
                  </div>
                </div>

                {/* Breakdown */}
                <div className="space-y-2.5 border-t border-[#27272a] pt-3">
                  {prizes.map((p) => (
                    <div key={p.place} className="flex items-center gap-2">
                      <span
                        className={`h-2 w-2 rounded-full ${placeColors[p.place] ? placeColors[p.place].replace('text-', 'bg-') : 'bg-slate-500'}`}
                      />
                      <span className="font-display font-semibold text-slate-200 text-sm">{p.amount}</span>
                      <span className="text-xs text-slate-500">{p.place}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4 border-t border-[#27272a] pt-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-display font-bold text-white text-lg">{submissions}</span>
                    <span className="block text-xs text-slate-500 uppercase tracking-wide">Submissions</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <Clock className="h-4 w-4 text-slate-500" />
                  <div>
                    <span className="font-display font-bold text-white text-lg">{remaining}</span>
                    <span className="block text-xs text-slate-500 uppercase tracking-wide">Remaining</span>
                  </div>
                </div>
              </div>
              ...
              {!isOwner && (
                <div className="space-y-3">
                  {claimError && <div className="text-red-600 text-xs font-semibold text-center mb-2">{claimError}</div>}
                  ...
                  {onChainStatus === 1 && !isClaimedByMe && (
                    <div className="text-amber-600 text-xs font-semibold text-center">This bounty has been claimed by another worker.</div>
                  )}
                  {/* Show success message if just submitted */}
                  {(submitSuccess || onChainStatus === 2) && (
                    <div className="mt-3 text-green-600 text-sm font-semibold text-center">✓ Work submitted — awaiting review.</div>
                  )}
                  {/* Approved */}
                  {onChainStatus === 3 && (
                    <div className="mt-3 text-emerald-600 text-sm font-semibold text-center">✓ Bounty approved & paid out.</div>
                  )}
                </div>
              )}
              ...
              {isOwner && (
                <div className="border-t border-border pt-5">
                  <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">You are the publisher</h4>
                  <p className="text-xs text-muted-foreground">Review submissions in the panel on the right.</p>
                </div>
              )}
              ...
              {bounty.region && (
                <div className="border-t border-border pt-5">
                  <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Regional Listing</h4>
              ...
              {skills.length > 0 && (
                <div className="border-t border-border pt-5">
                  <h4 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Skills Needed</h4>
            ...
            <div className="flex-1 p-6 md:p-8 overflow-y-auto max-h-[70vh]">
              ...
              {bounty.details ? (
                ...
                <p className="text-[15px] text-slate-300 leading-relaxed">{bounty.description}</p>
              )}
                ...
                <div className="mt-8">
                  ...
                  <ul className="space-y-2.5">
                    {bounty.requirements.map((req, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-[15px] text-slate-400">
              ...
              {isOwner && (
                <div className="mt-10 border-t border-[#27272a] pt-6">
                  ...
                  {responses.length === 0 ? (
                    <p className="text-sm text-slate-500 italic">No submissions yet. Share this bounty to attract participants!</p>
                  ) : (
                      ...
                      {responses.map((sub) => (
                        <div
                          ...
                          className={`rounded-xl border p-4 transition-colors ${
                            sub.status === 'approved' ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-[#27272a] bg-[#18181b]/50'
                          }`}
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 min-w-0">
                              <div className="h-8 w-8 shrink-0 rounded-full bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-[#27272a] flex items-center justify-center">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-sm text-slate-200 break-words">{sub.content}</p>
                            ...
                            <div className="shrink-0">
                              {sub.status === 'approved' ? (
                                ...
                                <Button
                                  size="sm"
                                  onClick={async () => {
                                    await handleApproveWork(sub.id)
                                  }}
                                  disabled={approving}
                                  className="bg-blue-600 hover:bg-blue-700 text-white text-xs h-8 px-3 disabled:opacity-50 shadow-lg shadow-blue-500/20"
                                >
                                  {approving ? (
                                    <span className="flex items-center gap-1">
                  ...
                  {approveResult && (
                    <motion.div
                      ...
                      className={`mt-4 rounded-xl border p-4 ${
                        approveResult.success ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-red-500/30 bg-red-500/10'
                      }`}
                    >
                      {approveResult.success ? (
                        <div>
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="font-bold text-emerald-300">Bounty Approved & Payment Sent!</span>
                          </div>
                          <div className="space-y-2 bg-[#09090b] border border-[#27272a] rounded-lg p-3 text-sm">
                          ...
                          <p className="text-xs text-emerald-500/70 mt-2">
                            The escrowed ALGO has been transferred to the worker&apos;s wallet.
                          </p>
                        </div>
                      ) : (
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <span className="font-bold text-red-400">Approval Failed</span>
                            <p className="text-sm text-red-400/80 mt-1 break-words">{approveResult.error}</p>
                          </div>
                        </div>
                      )}
                    </motion.div>
              ...
              {!isOwner && activeAddress && bounty.creatorAddress && (
                <div className="mt-10 border-t border-[#27272a] pt-6">
                  ...
                  {stateLoading && <p className="text-sm text-slate-500 italic">Checking bounty status...</p>}
                  ...
                  {!stateLoading && !isClaimedByMe && !submitSuccess && onChainStatus !== 2 && onChainStatus !== 3 && (
                    <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-center">
                      <p className="text-sm text-amber-500 font-medium">You must claim this bounty before you can submit work.</p>
                      <p className="text-xs text-amber-500/70 mt-1">Click "Claim Work" on the left panel first.</p>
                    </div>
                  )}
                  ...
                  {!stateLoading && onChainStatus === 2 && (
                    <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 text-center">
                      <p className="text-sm text-blue-400 font-medium">Work has already been submitted for this bounty.</p>
                      <p className="text-xs text-blue-400/70 mt-1">Waiting for the creator to review and approve.</p>
                    </div>
                  )}
                  ...
                  {!stateLoading && onChainStatus === 3 && (
                    <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
                      <p className="text-sm text-emerald-400 font-medium">This bounty has been approved and paid out.</p>
                    </div>
                  )}
                  ...
                  {!stateLoading &&
                    ...
                    !submitSuccess &&
                    onChainStatus === 1 &&
                    !responses.some((s) => s.submitter === activeAddress) && (
                      <>
                        <textarea
                          value={submitText}
                          ...
                          rows={4}
                          className="w-full rounded-xl border border-[#27272a] bg-[#18181b] px-4 py-3 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/30 resize-none transition-all"
                        />
                        {submitError && <div className="mt-2 text-red-400 text-xs font-semibold">{submitError}</div>}
                        <Button
                          onClick={handleSubmitWork}
                          disabled={!submitText.trim() || submitting}
                          className="mt-3 w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-lg shadow-blue-500/20 h-12 text-[15px]"
                        >
                  ...
                  {submitSuccess && (
                    <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
                      ...
                      <p className="text-xs text-emerald-500/70 mt-1">The bounty creator will review your submission.</p>
                    </div>
                  )}
                  ...
                  {responses.filter((s) => s.submitter === activeAddress).length > 0 && (
                    <div className="mt-6">
                      <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">Your Submissions</h4>
                      <div className="space-y-3">
                        {responses
                          .filter((s) => s.submitter === activeAddress)
                          .map((sub) => (
                            <div key={sub.id} className="rounded-lg border border-[#27272a] bg-[#18181b]/50 p-3">
                              <p className="text-sm text-slate-200 break-words">{sub.content}</p>
